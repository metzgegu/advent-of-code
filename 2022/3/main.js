...
const getResultPartOne = (tab) => {
  ...
  const pointsList = typeItemList.map(
    (item) => item.charCodeAt(0) - (item === item.toUpperCase() ? 38 : 96)
  )
...
const getResultPartTwo = (tab) => {
  ...
  for (let i = 0; i <= tab.length - 1; i += 3) {
    ...
    for (let i = 0; i <= part1.length - 1; i++) {
      if (
        part2.search(part1.at(i)) !== -1 &&
        part3.search(part1.at(i)) !== -1
      ) {
  ...
  const pointsList = resultTab.map(
    (item) => item.charCodeAt(0) - (item === item.toUpperCase() ? 38 : 96)
  )
