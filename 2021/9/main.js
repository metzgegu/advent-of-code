...
const getBassins = (i, j, matrix, alreadySeenMap) => {
  ...
  return [
    getValue(i, j, matrix),
    ...(!alreadySeenMap.get(`${i + 1},${j}`)
      ? getBassins(i + 1, j, matrix, alreadySeenMap)
      : []),
    ...(!alreadySeenMap.get(`${i},${j + 1}`)
      ? getBassins(i, j + 1, matrix, alreadySeenMap)
      : []),
    ...(!alreadySeenMap.get(`${i - 1},${j}`)
      ? getBassins(i - 1, j, matrix, alreadySeenMap)
      : []),
...
const isLowerPoint = (element, matrix, i, j) =>
  element < getValue(i + 1, j, matrix) &&
  element < getValue(i + 1, j + 1, matrix) &&
  element < getValue(i, j + 1, matrix) &&
  element < getValue(i - 1, j, matrix) &&
  element < getValue(i - 1, j - 1, matrix) &&
  element < getValue(i, j - 1, matrix)

const getLowerPoints = (matrix) =>
  matrix.flatMap((line, i) =>
    line.filter((element, j) => isLowerPoint(element, matrix, i, j))
  )
