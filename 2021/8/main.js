...
const getNumber = (value, digitMapping) => {
  ...
  return digitMapping.filter(
    (v) =>
      v.letters.length === value.length &&
      value.split('').every((l) => v.letters.includes(l))
  )[0]?.value
}

const getDigitMapping = (patterns) => {
  const isEqual = (e0, e1) =>
    e0.split('').every((e) => e1.split('').includes(e))
  ...
  const two = patterns
    .filter((e) => e.length === 5 && !isEqual(e, three))
    .filter((pattern) => [...new Set([...four, ...pattern])].length === 7)[0]
  const five = patterns.filter(
    (e) => e.length === 5 && !isEqual(e, three) && !isEqual(e, two)
  )[0]
  const zero = patterns.filter(
    (e) =>
      e.length === 6 && [...new Set([...five, ...e.split('')])].length === 7
  )[0]
  const six = patterns.filter(
    (e) => e.length === 6 && !isEqual(e, zero) && !isEqual(e, nine)
  )[0]
